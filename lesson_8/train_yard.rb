...
class TrainYard
  ...
  def add_route
    ...
    if opt == '1'
      ...
      route = Route.all.find { |r| from == r.way.first } # to_s!
  ...
  def assign_route
    ...
    from  = ask 'Введите маршрут: '
    route = Route.all.find { |r| from == r.way.first }
  ...
  def coupling
    ...
    if train.type == :pass
      ...
      train.attach(car: PassengerCar.new(number: car_number, seats: seats.to_i))
    else
      volume = ask 'Укажите объём вагона: '
      train.attach(car: CargoCar.new(number: car_number, volume: volume.to_i))
    end
  rescue RuntimeError => e
    puts e.message
  end
  ...
  def seat
    ...
    car = train.car(number: car_number.to_i)
